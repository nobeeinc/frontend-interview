      ...
      <div className='mx-5 mt-6'>
        ...
        <Formik
          ...
          onSubmit={(
            ...
            if (values.password === values.passwordConfirm) {
              axios.post(`http://localhost:3000/api/auth/register`, { email: values.email, password: values.password }
              )
                .then((res) => {
        ...
        >
          <Form >
            ...
            <FormField id="passwordConfirm" label="Confirm password" name="passwordConfirm" additionalClass={errMessage ? 'border-danger' : 'border-primary-200'} onShow={confirmPasswordType}></FormField>
        ...
        <button className='absolute right-6 top-72' onClick={confirmPasswordHandler}>
          {confirmPasswordType === 'password' ? <EyeIcon className='w-6 mb-4 font-thin'></EyeIcon> : <EyeInvisibleIcon className='w-6 mb-4 font-thin' />}
        </button>
        <div className='font-sans text-sm mt-2'>Already have an account? <span className='font-sans font-semibold text-sm underline ' onClick={() => { router.push('/login') }}>Login</span></div>
      </div>
    </div >
  )
}
